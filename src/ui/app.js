...
async function resumeSaved() {
  // ★ **브릿지를 지나간다**(2026-09-02 · 사용자 결정). 앞서 `app.gate`를
  //   세워 브릿지를 건너뛰고 `routeGo()`로 곧장 결과에 닿았다 — 그러면
  //   **얼마나 지났는지 모른 채 도착한다.** 이 버튼을 누르는 사람은 답을
  //   다시 걷다가 마음을 바꾼 사람이고, 그에게도 경과 시간과 "지금 시점에
  //   맞게 다시 정리한다"는 말이 필요하다.
  //
  //   **별도 변형을 만들지 않는다** — 재방문 진입 때와 같은 화면이고 같은
  //   히스토리 규칙이다(랜딩·브릿지 둘 다 CONSUMED라 떠날 때 덮는다).
  //   `gate`를 내리면 `route()`가 브릿지를 고르고, 그 CTA(`passGate`)가
  //   다시 `routeGo()`로 분기한다 — 새 질문이 있으면 설문, 없으면 도착.
  app.gate = false;
  app.again = false;
  await leaveLanding(routeGo);
}
