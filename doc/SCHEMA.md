...
| `fire_investigation` | object | 관할 소방서 화재조사 직통(아래) |
...
### 관할 소방서 화재조사 직통 — `fire_investigation`
...
"fire_investigation": {
  ...
  "verified_at_url": null
}
```

**화재증명원 발급과 조사 진행을 묻는 유일한 직통이다.** 25개 구 전수이고
구마다 다른 번호다. 구청 대표번호는 싣지 않는다 — 걸면 120으로 연결되어
도달점이 같고 유지보수만 는다. 소방서 대표(민원) 번호도 이 직통으로
대체된다.

**중구의 관할만 이름이 다르다**(중부소방서). 나머지 24구는 `{구 이름}소방서`다.

**`verified_at_url`이 25건 전부 `null`이다.** 출처는 사용자 제공 자료이고,
`fire.seoul.go.kr`의 소방서 조직 페이지는 본문을 JS로 그려서 어떤 경로로도
번호를 읽지 못했다(4KB 셸만 온다). **확인하지 못한 것을 확인한 것처럼 적지
않는다** — `test/districts.test.mjs` §5-b가 그 사실을 박아 두어서, 나중에
검증이 되면 그 검사가 먼저 알려 준다.

**변동 가능성이 대표번호보다 크다.** 팀 단위 직통이라 조직개편 때 바뀐다
(유지보수 주기는 아래).
...
| 항목 | 주기 | 사유 |
|---|---|---|
| 화재조사 직통 25개 | 조직개편 시 | 팀 단위 직통이라 대표번호보다 변동이 크다 |
| 긴급복지 금액·소득·재산 기준 | 매년 1~2월 | 연도별로 갱신된다(`emergency-welfare` 본문) |
| 서울시민안전보험 번호 | 매년 1월 | 담당 보험사 컨소시엄이 해마다 바뀐다(아래) |
| 119·112·120·129·132·1372·1332·1577-0199·1670-9512 | 고정 | 대표번호 |

**서울시민안전보험(1522-3556)은 싣지 않았다.** 그 번호의 노출 조건이
**인명피해가 있을 때**인데 — 재산피해를 보장하지 않아서, 조건 없이 띄우면
"집이 탔다"만으로 전화했다가 거절당하는 경로가 생긴다 — **설문에 인명피해를
묻는 질문이 없다.** 조건을 만들 수 없으므로 백로그로 둔다. 질문이 생기면
그때 조건과 함께 싣는다.
...
**★ 그 `default`는 판정용 추론이지 질문 게이트가 아니다.**
`product_suspected`의 `default: "unknown"`은 "발화 위치를 모르면 제품
여부도 모른다"는 의도된 추론이고, 그 값이 Action 층에서 보존·인계 안내를
켜는 것이 맞다(D-013 — 모르는 사람이 증거를 버리면 되돌릴 수 없다).
**그러나 그 값이 다른 질문의 `ask_when`을 열어서는 안 된다** — 부모를
건너뛴 사람에게 자식 질문이 뜬다. 그래서 조건부 질문의 키를 참조하는
자식은 **부모의 게이트를 함께 실어야 한다**(`q-maker`가 `product_suspected`와
`origin_area`를 함께 보는 이유). `test/questions.test.mjs` §8이 전 질문을
순회하며 이것을 지킨다 — 원시 state와 판정 사본 양쪽에서 본다.
