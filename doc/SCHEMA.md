...
| `sources` | [object] | 구조화된 출처. **59건 중 1건만 채워져 있다**(아래) |
| `contacts` | [object] | 연락처. **59건 중 5건만 채워져 있다**(아래) |
| `checked_at` | date | |
...
**지금 채워진 것은 1건이다** — `fire-loss-compensation-not-applicable`
(소방기본법 제49조의2 · 시행령 제11조. 사용자가 원문을 확인해 전달했다).
나머지 58건을 채우는 것은 원문을 하나씩 확인하는 별도 콘텐츠 패스다 —
**URL이나 본문 문자열을 파싱해 법령명·조문을 만들어 내지 않는다.**
없는 것을 지어내느니 출처 영역을 안 그리는 편이 낫다.
...
**지금 채워진 것은 5건이다**(2026-08-30 · 1군). 국가 전문기관 창구가
유일해서 창구 선택 딜레마가 없는 것들이다 —
...
**화면에는 아직 소비자가 없다.** 행 계약(`rows.js`)에 싣지 않았고 노출
방식은 시각 디자인 패스가 정한다. `copy.js`의 `CONTACT_BY_ACTION`은
라우팅에서 분리된 잔존 코드이고, 정리는 그 패스의 몫이다.
