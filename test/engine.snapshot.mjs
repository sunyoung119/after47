...
const PERSONAS = [
  ...
  // P21~P24는 자치구가 5개에서 25개로 늘어난 뒤의 자리다(5단계-0).
  // 전수 검증은 districts.test.mjs가 하므로 여기에는 **경로가 다른 대표만**
  // 넣는다 — 20개를 그대로 밀어넣으면 기준선이 커지기만 하고 새로 보이는
  // 것이 없다. 넷을 고른 기준은 "기존 20페르소나가 한 번도 안 밟은 조합".
  //
  //   P21 emergency_exception이 제외를 조건부로 바꾸는 것
  //   P22 미니멀형 조례 (지원 항목이 하나)
  //   P23 주택한정 + 기한 + 긴급예외가 한 구에 겹친 것
  //   P24 신규 미보유 구
  //
  // P21이 P11(양천)과 나란히 읽힌다. 같은 compensated인데 양천은 `제외`,
  // 관악은 `조건부`다 — 차이를 만드는 것은 emergency_exception 하나다.
  // 관악의 보험 조문은 원문이 「가입되어 보험금을 지급받는 경우」로 가입과
  // 수령의 결합이라 현행 enum에 정확히 안 맞는다. 수령 요건으로 근사했고
  // 그 사실은 districts.json의 exclusion_note에 남아 있다.
  ["P21", "관악 — 보험금 수령 (compensated + 긴급 예외 → 제외가 아니라 조건부)",
    { district: "gwanak", compensated: true }],

  // P22는 조례가 있는데 지원 항목이 임시주거 하나뿐인 구다. 긴급급식도
  // 조문에 있지만 대응 Action이 없다(집행 미확인. D-020 초안) — 조례 행이
  // 1건으로 보이는 것이 맞다. 제외 조건은 중복지원 둘뿐이라 보험을 안 본다.
  ["P22", "송파 — 미니멀형 조례 (제외 조건 없음, 조례 행 1건)",
    { district: "songpa" }],

  // P23은 세 조건이 한 구에 겹친 유일한 자리다. +90d부터 기한 30일이
  // 도과하는데 긴급 예외가 있어 `제외`가 아니라 `조건부`로 뜬다.
  // 양천(P11·P20)은 같은 30일인데 예외가 없어 `제외`다.
  ["P23", "영등포 — 주택한정 + 기한 30일 + 긴급 예외",
    { district: "yeongdeungpo" }],

  // P24는 신규 미보유 11개 구가 마포와 같은 경로를 타는지 보는 자리다.
  // fallback 문구는 마포에만 있고(그 구의 사회재난 조례를 실제로 확인한
...
// +14d는 R5의 사실 축 7일 경계를 관측하기 위한 것이다. +5d와 +90d
// 사이가 비어 있으면 7일(사실)과 30일(제도) 경계의 차이가 나타나는
// 조합이 없어, 둘 중 하나를 잘못 구현해도 계기판이 못 잡는다.
const CLOCKS = [
  ["+3h", 3],
  ["+5d", 5 * 24],
  ["+14d", 14 * 24],
