...
//   답을 다시 걸으려다 마음이 바뀐 사람을 돌려보내는 것 하나다.
{
  ...
  t("[상황 다시 알리기]로 왔고 저장 기록이 있으면 되돌아가기가 뜬다",
...
t("게이트 문구가 확정 두 줄이다",
  게이트.lines.join("\n") === "지금 시점에 맞는 안내로\n다시 정리합니다.",
  게이트.lines.join(" / "));
// ★ **라벨 = 도착지 이름**(사용자 결정). 브릿지와 전환이 같은 자리에
//   닿으므로 같은 말을 쓴다 — 한 상수를 나눠 쓰는지도 함께 본다.
t("게이트 CTA가 도착지 이름이다", 게이트.cta === "내 회복 경로 보기", 게이트.cta);
t("전환 CTA와 같은 문구다", 게이트.cta === COPY.transition.cta);
...
t("우상단이 '상황 다시 알리기'다", 게이트.home === "상황 다시 알리기", 게이트.home);
