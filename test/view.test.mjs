...
// 눈으로 지킬 수 있는 종류가 아니라서 규칙의 존재를 검사로 박는다.
{
  ...
  // ── 구간은 흰 카드, 펼친 안내는 파란 면 (2026-09-02 · 보라 폐기) ──
  //
  // 면의 자리가 뒤집혔다. **색면은 하나뿐이어야 한다** — 둘 다 색면이면
  // 무엇이 층의 주인공인지 흐려지고, 둘 다 흰 면이면 갈리지 않는다.
  // 파란 면의 색은 `나를 위한 안내 보기` 버튼과 **같은 토큰**이다:
  // 누르면 안내로 들어가는 면은 앱 전체에서 이 색 하나다.
  {
    const tk = readFileSync(join(D, "src/ui/tokens.css"), "utf8");
    const 값 = (name) =>
    ...
    // 구간 요약 = 흰 카드. 사진 배경 위라 **불투명**이어야 한다.
    const 공유 = (css.match(/\.tline__sum,\s*\.tline__info\s*\{[^}]*\}/) || [""])[0];
    t("구간 요약이 흰 카드 면이다", /background:\s*var\(--c-surface\)/.test(공유), 공유.replace(/\s+/g, " "));
    t("구간 요약이 카드 테두리를 쓴다", /border:\s*1px solid var\(--c-line-strong\)/.test(공유));
    t("구간 요약이 면 색을 직접 지정하지 않는다 (글자는 본문색을 상속한다)",
      !/color:/.test(공유), 공유.replace(/\s+/g, " "));
    // 흰 면 위 본문·뮤트는 이미 AA다. 바탕과 갈리는지만 본다(비텍스트 3:1은
    ...
    t("화면 규칙에도 보라 잔재가 없다", !/[-][-]c-band/.test(css));
  }

  // 파란 면 위에서 **기본 배지**는 1.3까지 떨어진다. 지금 타임라인 카드가
  // 담는 것은 제목·요약·화살표 셋뿐이라 문제가 없는데, 배지가 생기면
  // 규칙을 함께 만들어야 한다 — 그 전제가 깨지면 여기서 걸린다.
  {
    const 배지 = 결과(전.state).timeline.nodes
      .flatMap((n) => n.items || [])
      .filter((it) => it.stateLabel || it.statusLabel);
    t("타임라인 카드에 배지가 없다 (파란 면 위 기본 배지는 1.3이다)",
