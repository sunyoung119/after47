...
//   다시 잠겼다.** 실측으로 잡은 것이라 여기서 함께 지킨다.
{
  ...
  // **무엇을 비워도 되는지**를 말한다 — 날짜는 필수다.
t("시각은 비워도 된다는 한 줄이 있다", has(main(), "시각은 모르면 비워두셔도 됩니다."));
...
// 위에 있으면 경로보다 먼저 읽혀 도착 화면이 경유지로 보인다.
{
  ...
  t("허브로 가는 문이 타임라인 아래에 있다",
    ...
    순.slice(0, 6).join(" | "));
  // ★ 설명은 **문 아래**다(2026-09-02 · 사용자 결정). 앞문장이 가리키는
  //   체크리스트로 가는 길을 뒷문장이 적는데, 그 길이 바로 위 버튼이다 —
  //   위에 붙이면 버튼을 밀어 내리고 도착 화면의 첫 화면이 글이 된다.
  {
    const 설명 = 순.findIndex((x) => x.includes("체크리스트는 [나를 위한 안내 보기]"));
    t("체크리스트로 가는 길이 화면에 적혀 있다", 설명 >= 0, 순.slice(-2).join(" | "));
    t("그 설명이 버튼 아래에 온다", 설명 > 순.indexOf("나를 위한 안내 보기"),
      `버튼 ${순.indexOf("나를 위한 안내 보기")} / 설명 ${설명}`);
  }
  문.click();
}
await tick(30);
...
// ── [이전] 대조 (규칙) ─────────────────────────────
//
// **[이전]이 그려진 자리는 전부 기기 뒤로가기와 같은 곳에 닿아야 한다.**
// 자리마다 검사를 다는 대신 순회 한 벌을 돌린다 — 화면이 늘어도 목록에
// 한 줄이면 끝나고, 빠뜨리면 그 자리가 통째로 사각이 된다.
const 자리문자열 = () => texts(main()).slice(0, 4).join(" | ");
const 이전버튼 = () => [...$("top-right").children].find((n) => n.textContent === "이전") ?? null;
const 허브인가 = () => Boolean(카드("체크리스트"));

// 어디에 있든 허브로 돌아온다 — 순회의 출발선을 매번 같게 만든다.
async function 허브로() {
...
// 한 자리를 잰다 — 도달해서 [이전]을 누른 결과와, 다시 도달해서 기기
// 뒤로가기를 누른 결과가 같은가.
async function 대조(이름, 도달) {
  await 도달();
  const 버튼 = 이전버튼();
...
function 결과자리들() {
  ...
  const 상세로 = (이름, 고르기) => async () => {
    await 한걸음(이름)();
    고르기();
    await tick(10);
  };
  const 첫항목 = () => all(main(), (n) => hasClass(n, "card__hit"))[0]?.click();
  const 첫주제 = () => all(main(), (n) => hasClass(n, "tcard"))[0]?.click();
  return [
    ["허브", 허브로],
    ...["먼저 볼 내용", "체크리스트", "알아둘 내용", "내 회복 경로", "주제별 보기", "근거 법령", "연락처"].map(
      (n) => [n, 한걸음(n)]
    ),
    ["체크리스트 → Action 상세", 상세로("체크리스트", 첫항목)],
    ["먼저 볼 내용 → Action 상세", 상세로("먼저 볼 내용", 첫항목)],
    ["주제별 보기 → 주제 상세", 상세로("주제별 보기", 첫주제)],
  ];
}

// 설문 계열은 저장소를 새로 만들고 처음부터 걷는다 — 허브에서 한 걸음이
// 아니라서 도달이 비싸지만, 이 자리들이야말로 사람이 제일 많이 누른다.
async function 새설문() {
  configureStorage({ ...memoryBackend(), readJson });
  await 열기();
  button($("intro"), "회복 시작하기").click();
  await tick(30);
...
//
// 그래서 검사를 **한 화면이 아니라 규칙으로** 둔다: [이전]이 그려진 자리를
// 전부 돌며 둘을 나란히 눌러 비교한다.
//
// ★ **순회 대상은 결과 화면 다섯이 아니라 [이전]이 서는 자리 전부다**
//   (2026-09-01 확장). 앞서는 결과 화면만 돌아서, 설문·전환·상세처럼
//   [이전]이 이미 서 있던 자리는 아무도 안 보고 있었다. 화면이 늘면
//   여기 한 줄을 더하는 것이 규칙이다.
{
  for (const [이름, 도달] of 결과자리들()) await 대조(이름, 도달);
  // 뒤따르는 검사들은 허브에서 시작한다 — 순회가 끝난 자리를 되돌려 놓는다.
  await 허브로();
}
...
// ── ⑤-b 설문 계열의 [이전] ─────────────────────────
section("⑤-b 설문·전환·안내 범위 — [이전]이 기기 뒤로가기와 같은가");

// 위 순회와 **같은 규칙**을 설문 계열에 돌린다. 여기는 허브에서 한 걸음이
// 아니라 매번 처음부터 걸어야 해서 자리를 나눴을 뿐, 보는 것은 하나다.
{
  const 세번째 = async () => {
    await 새설문();
    await 한문항();
  ...
  await 대조("설문 세 번째 질문", 세번째);
  await 대조("안내 범위", 안내범위);
  await 대조("전환", 전환);
}

// ── ⑤-c 설문을 [이전]으로 거슬러 오르기 ─────────────
section("⑤-c 거슬러 올라도 답은 그대로다");

// **답 비파괴가 이 화면의 계약이다.** 되돌아가면 고른 답이 선택 상태로
// 보이고, 바꾸면 그때부터 뒤가 정리된다(pruneStale). 앞으로 걸을 때
// 건너뛴 질문은 **거슬러 올라도 나오지 않는다** — 조건부 스킵이 역방향에도
// 그대로 산다. 왕복 한 벌로 셋을 함께 본다.
{
  await 새설문();
  const 앞으로 = [];
  for (let i = 0; i < 30; i++) {
  ...
  t("설문을 끝까지 걸어 전환에 닿았다", 앞으로.length >= 10 && has(main(), "확인했습니다"),
    `${앞으로.length}문항 · ${texts(main()).slice(0, 2).join(" | ")}`);
  t("앞으로 걸을 때 제품 질문이 나왔다 (발화 지점이 '우리 집 안')",
    앞으로.includes("불이 사용하던 제품에서 시작됐다고 들으셨나요?"), 앞으로.join(" > "));

  // 전환의 [이전]으로 설문 끝에 들어가 거슬러 오른다.
  이전버튼().click();
  await tick(40);
  const 뒤로 = [];
  const 답표시 = [];
  for (let i = 0; i < 30; i++) {
    ...
    if (!q) break;
    뒤로.push(q.own);
    답표시.push(all(main(), (n) => hasClass(n, "q__choice--on")).length);
    const b = 이전버튼();
    if (!b) break;
    b.click();
    await tick(40);
  }
  t("거슬러 오른 순서가 걸어온 순서의 역순이다",
    JSON.stringify([...뒤로].reverse()) === JSON.stringify(앞으로),
    `앞으로 ${앞으로.length} / 거슬러 ${뒤로.length}`);
  t("건너뛴 질문은 거슬러도 나오지 않는다",
    뒤로.every((x) => 앞으로.includes(x)),
    뒤로.filter((x) => !앞으로.includes(x)).join(" | ") || "(누수 없음)");
  t("되돌아간 질문마다 고른 답이 선택 상태다",
    답표시.length > 0 && 답표시.every((n) => n === 1), 답표시.join(","));
  t("첫 질문의 [이전]은 기본 확인이다", has(main(), "화재가 있었던 날짜와 지역을 알려주세요"),
    texts(main()).slice(0, 2).join(" | "));
  t("기본 확인에는 [이전]이 없다 (온 길이 덮여 있다)", 이전버튼() === null,
    texts($("top-right")).join(","));

}

// 되돌아가 **답을 바꾸면** 그때부터 뒤가 정리된다(pruneStale + ask_when).
// 앞 블록에서 이어 하지 않는다 — 거기서는 이미 다 답한 상태라 기본 확인의
// [다음]이 설문을 통째로 건너뛰고 결과로 간다(route). 새로 걸어서 본다.
{
  await 새설문();
  const 발화 = "불이 어디에서 시작됐는지 들으셨나요?";
  const 제품 = "불이 사용하던 제품에서 시작됐다고 들으셨나요?";
  for (let i = 0; i < 30; i++) {
    const q = 질문중();
    if (!q) break;
    const 지금 = q.own;
  ...
  t("발화 지점을 '우리 집 안'으로 답하면 다음이 제품 질문이다", 질문중()?.own === 제품,
    질문중()?.own ?? texts(main()).slice(0, 2).join(" | "));

  이전버튼().click();
  await tick(40);
  t("[이전]이 발화 지점 질문으로 되돌린다", 질문중()?.own === 발화,
    질문중()?.own ?? texts(main()).slice(0, 2).join(" | "));
  const 고른것 = all(main(), (n) => hasClass(n, "q__choice--on"));
  t("고른 답이 그대로 선택되어 있다 (답 비파괴)",
    고른것.length === 1 && 고른것[0].textContent.includes("우리 집 안"),
    고른것.map((n) => n.textContent).join(","));

  await 한문항({ [발화]: "아직 모르거나 듣지 못했어요" });
  t("답을 바꾸면 바로 다음이 제품 질문이 아니다 (pruneStale + ask_when)",
    질문중() !== null && 질문중().own !== 제품,
    질문중()?.own ?? texts(main()).slice(0, 2).join(" | "));
  ...
  t("그 뒤로 제품 질문도 제조사 질문도 안 나온다",
    남은.length > 0 && !남은.includes(제품) && !남은.includes("제품 제조사에서 연락이 왔나요?"),
    남은.join(" > "));
}
...
// 랜딩까지 왔다가 마음을 바꾼 사람의 출구이기도 하다.
{
  ...
  t("④ 다시 알리기로 온 랜딩에 되돌아가기가 있다", Boolean(보조),
    texts($("intro")).join(" | "));
...
// ★ **보조 버튼도 브릿지를 지난다**(2026-09-02 · 사용자 결정). 앞서
// `routeGo()`로 곧장 결과에 닿았다 — 그러면 얼마나 지났는지 모른 채
// 도착한다. 이제 재방문 진입과 **같은 화면·같은 히스토리 규칙**을 쓰고,
// 브릿지 CTA가 다시 `routeGo()`로 분기한다.
//
// 그래서 이 절이 보는 것은 셋이다 — ①보조 버튼을 누르면 **브릿지가 서는가**
// ②그 CTA를 지난 최종 도착이 재방문 직접 경로와 **같은가** ③새 질문이
// 생긴 시점에도 같은가(앞서 이 문이 직행하며 **화재 7일 뒤 조사서 질문을
// 건너뛰던** 자리다).
{
  ...
  const 밟기 = {
    ...
    async 보조() {
      ...
      b.click();
      await tick(40);
      // ★ 여기서 브릿지가 서야 한다 — 재방문 진입과 같은 화면이다.
      브릿지본것 = has(main(), "화재 발생 후") && Boolean(결과로());
      const 문 = 결과로();
      if (!문) return `(브릿지 CTA 없음) ${자리()}`;
      문.click();
      await tick(40);
      return 자리();
    },
  ...
  // ⓐ 남은 질문이 없을 때
  {
    ...
    const 보 = await 밟기.보조();
    t("ⓐ 보조 버튼을 누르면 브릿지가 선다", 브릿지본것, `보조: ${보}`);
  ...
  // ⓑ 화재 7일이 지나 조사서 질문이 새로 생겼을 때
  {
    ...
    const 보 = await 밟기.보조();
    t("ⓑ 새 질문이 생겨도 보조 버튼은 브릿지를 지난다", 브릿지본것, `보조: ${보}`);
  ...
  // ⓒ **도착 화면이 '지금' 기준으로 다시 계산되는가.**
  //
  // 브릿지 CTA의 라벨이 `내 회복 경로 다시 계산하기`다 — 하는 일을 적은
  // 문이므로, 실제로 다시 계산되는지를 밟아서 본다. 앞의 ⓐ·ⓑ는 **어디에
  // 닿는가**를 봤고 여기는 **닿은 화면의 내용이 달라지는가**를 본다.
  //
  // 답을 하나도 바꾸지 않고 `fire_at`만 뒤로 미는 것은 **같은 사람이 며칠
  // 뒤에 다시 여는 것**과 같다(경과 시간은 둘의 차이로만 계산된다).
  // 저장된 판정이 있다면 여기서 옛 화면이 나온다 — 엔진은 매 렌더마다
  // 처음부터 다시 돌고, 그 사실이 이 서비스의 본체다.
  {
    const 요약 = () => all(main(), (n) => hasClass(n, "tline__count")).map((n) => n.own).join(" / ");
    const 도착 = async (일) => {
      되감기(일);
      await 열기();
      결과로().click();
    ...
    const 당일 = await 도착(0);
    const 사흘뒤 = await 도착(3);
    t("ⓒ 답이 같아도 '지금'이 달라지면 구간 개수가 달라진다",
      당일 !== 사흘뒤 && 당일.length > 0 && 사흘뒤.length > 0,
      `당일: ${당일}  ||  사흘 뒤: ${사흘뒤}`);
    // 골든타임 항목(4h·6h·24h·48h)이 시한을 지나면 `늦었어도 확인하세요`로
    // 내려간다. **당일에는 없고 사흘 뒤에는 있다** — placement가 지금 시각을
    // 본다는 가장 눈에 띄는 증거다.
    const 지나간것 = async () => {
      buttonLike(main(), "나를 위한 안내 보기").click();
      await tick(20);
      카드("먼저 볼 내용").click();
      await tick(20);
      return has(main(), "늦었어도 확인하세요");
    };
    const 사흘뒤지나감 = await 지나간것();
    await 도착(0);
    const 당일지나감 = await 지나간것();
    t("ⓒ 당일에는 지나간 것이 없고 사흘 뒤에는 있다",
      사흘뒤지나감 === true && 당일지나감 === false,
      `당일: ${당일지나감} / 사흘 뒤: ${사흘뒤지나감}`);
  }
}
