...
t("타임라인 우상단이 [상황 다시 알리기] 하나다 (도착 화면에는 뒤가 없다)",
  texts($("top-right")).join(",") === "상황 다시 알리기",
...
t("허브 우상단이 [이전]과 [상황 다시 알리기]다",
  texts($("top-right")).join(",") === "이전,상황 다시 알리기",
  texts($("top-right")).join(" | "));
...
// ── ⑥ 처음으로 ────────────────────────────────────
section("⑥ 상황 다시 알리기 — 브릿지에서 답을 다시 걷는다");
...
t("① 브릿지 우상단에 [상황 다시 알리기]가 있다",
  $("top-right").children[0]?.textContent === "상황 다시 알리기",
...
t("① 재설문을 마치면 허브이고 우상단에 [상황 다시 알리기]가 있다",
  has(main(), "나를 위한 안내") &&
    texts($("top-right")).includes("상황 다시 알리기"),
  texts($("top-right")).join(" | "));
